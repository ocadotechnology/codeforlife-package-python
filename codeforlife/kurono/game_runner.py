...
from .django_communicator import (
    DjangoCommunicator,
...
class GameRunner:
    def __init__(
        ...
        self.simulation_runner = ConcurrentSimulationRunner(communicator=self.communicator, game_state=self.game_state)
    ...
    def get_users_to_add(self, game_metadata):
        def player_is_new(_player):
            return _player["id"] not in self.simulation_runner.game_state.avatar_manager.avatars_by_id.keys()
