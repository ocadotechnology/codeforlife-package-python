...
from ..interactables.score_location import ScoreLocation
...
class ScoreLocationUpdater(_MapUpdater):
    def update(self, world_map, context):
        ...
        target_num_score_locations = int(
            math.ceil(context.num_avatars * world_map.settings["TARGET_NUM_SCORE_LOCATIONS_PER_AVATAR"])
        )
...
class PickupUpdater(_MapUpdater):
    ...
    def update(self, world_map, context):
        target_total_num_pickups = context.num_avatars * world_map.settings["TARGET_NUM_PICKUPS_PER_AVATAR"]
        target_num_pickups_per_type = int(math.ceil(target_total_num_pickups / len(self.pickup_types)))
        current_num_pickups = Counter(type(cell.interactable) for cell in world_map.pickup_cells())
        for pickup_type in self.pickup_types:
            max_num_pickups_to_add = target_num_pickups_per_type - current_num_pickups[pickup_type]
